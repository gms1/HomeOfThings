...
import { ContactEntityManagerService } from './fixtures/service/contact.entity-manager.service';
import { UserEntityManagerService } from './fixtures/service/user.entity-manager.service';
import * as init from './fixtures/init';
import { User } from './fixtures/entity/user';

const CONNECTION_OPTIONS: Sqlite3ConnectionOptions = {
  file: 'file:entity-manager.spec.db?mode=memory&cache=shared',
};

describe('Entity-Manager-Integration', () => {
  let appModule: TestingModule;
  let connectionManager: ConnectionManager;
  let userService: UserEntityManagerService;
  let contactService: ContactEntityManagerService;

  beforeAll(async () => {
    appModule = await Test.createTestingModule({
      // NOTE: Sqlite3Module.forFeature([User, Contact]) is required to make AutoUpgrade in init.ts happy (see ConnectionManager.getTables in ./fixtures/init)
      imports: [Sqlite3Module.register(CONNECTION_OPTIONS), Sqlite3Module.forFeature([User, Contact])],
      providers: [UserEntityManagerService, ContactEntityManagerService],
    })
    ...
    userService = appModule.get(UserEntityManagerService);
    expect(userService).toBeInstanceOf(UserEntityManagerService);
    contactService = appModule.get(ContactEntityManagerService);
    expect(contactService).toBeInstanceOf(ContactEntityManagerService);
