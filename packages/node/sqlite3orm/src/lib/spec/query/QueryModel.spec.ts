...
describe('test QueryModel', () => {
  ...
  it('`eq` Promise<string> predicate (shorthand form)', async () => {
    try {
      ...
      const res = await userDao.selectAll({
        userLoginName: Promise.resolve('Charlie'),
      });
      expect(res.length).toBe(1);
      expect(res[0].userId).toBe(3);
    } catch (err) {
  ...
  it('undefined `neq` comparison value should be treated as `null` value (empty result set)', async () => {
    try {
      const userDao = new BaseDAO(User, sqldb);
      const res = await userDao.selectAll({
        userLoginName: { neq: undefined },
      }); // this is NOT equivalent to: IS NOT NULL!!!
  ...
  it('`isBetween` [string,string] predicate', async () => {
    try {
      const userDao = new BaseDAO(User, sqldb);
      const res = await userDao.selectAll({
        userLoginName: { isBetween: ['Alfa', 'Bravo'] },
      });
      expect(res.length).toBe(2);
    } catch (err) {
      failTest(err);
  ...
  it('`isNotBetween` [string,string] predicate', async () => {
    try {
      const userDao = new BaseDAO(User, sqldb);
      const res = await userDao.selectAll({
        userLoginName: { isNotBetween: ['Charlie', 'Z'] },
      });
      expect(res.length).toBe(2);
    } catch (err) {
      failTest(err);
  ...
  it('`isNotIn` string[] predicate', async () => {
    try {
      ...
      const res = await userDao.selectAll({
        userLoginName: {
          isNotIn: ['Bravo', 'Charlie', 'Delta', 'Echo', 'Foxtrot'],
        },
      });
      expect(res.length).toBe(1);
      expect(res[0].userId).toBe(1);
  ...
  it('`isNotIn` number[] predicate', async () => {
    try {
      const userDao = new BaseDAO(User, sqldb);
      const res = await userDao.selectAll({
        userFollowers: { isNotIn: [5, 4, 3, 2, 1] },
      });
      expect(res.length).toBe(1);
      expect(res[0].userId).toBe(1);
    } catch (err) {
  ...
  it('`eq` Date (INT) predicate (shorthand form)', async () => {
    try {
      const userDao = new BaseDAO(User, sqldb);
      const res = await userDao.selectAll({
        userCreated: new Date('2018-01-03'),
      });
      expect(res.length).toBe(1);
      expect(res[0].userId).toBe(3);
    } catch (err) {
  ...
  it('`eq` Date (INT) predicate (normal form)', async () => {
    try {
      const userDao = new BaseDAO(User, sqldb);
      const res = await userDao.selectAll({
        userCreated: { eq: new Date('2018-01-04') },
      });
      expect(res.length).toBe(1);
  ...
  it('`isBetween` Date (INT) predicate', async () => {
    try {
      ...
      const res = await userDao.selectAll({
        userCreated: {
          isBetween: [new Date('2018-01-03'), new Date('2018-01-04')],
        },
      });
      expect(res.length).toBe(2);
  ...
  it('`isNotBetween` Date (INT) predicate', async () => {
    try {
      ...
      const res = await userDao.selectAll({
        userCreated: {
          isNotBetween: [new Date('2018-01-03'), new Date('2018-01-04')],
        },
      });
  ...
  it('`eq` Date (TEXT) predicate (shorthand form)', async () => {
    try {
      const userDao = new BaseDAO(User, sqldb);
      const res = await userDao.selectAll({
        userUpdated: new Date('2018-01-03'),
      });
      expect(res.length).toBe(1);
      expect(res[0].userId).toBe(4);
    } catch (err) {
  ...
  it('`eq` Date (TEXT) predicate (normal form)', async () => {
    try {
      const userDao = new BaseDAO(User, sqldb);
      const res = await userDao.selectAll({
        userUpdated: { eq: new Date('2018-01-04') },
      });
      expect(res.length).toBe(1);
      expect(res[0].userId).toBe(3);
    } catch (err) {
  ...
  it('`selectOne` should succeed', async () => {
    try {
      const userDao = new BaseDAO(User, sqldb);
      const res = await userDao.selectOne({
        userCreated: new Date('2018-01-03'),
      });
  ...
  it('selectOne should fail for emtpy results', async () => {
    try {
      const userDao = new BaseDAO(User, sqldb);
      const res = await userDao.selectOne({
        userLoginName: { eq: `not a login name` },
      });
    } catch (err) {
      return;
    }
