...
import { SQL_MEMORY_DB_SHARED, SQL_OPEN_DEFAULT, SQL_OPEN_READWRITE, SqlConnectionPool, SqlDatabase, SqlDatabaseSettings } from '../../';
...
describe('test SqlConnectionPool', () => {
  ...
  it('expect pool to share a file-database', async () => {
    try {
      ...
      try {
        ...
        failTest('got 3 connection from pool with max 2 connections');
      } catch (ignore) {}
  ...
  it('expect pool to be closed after failed attempt to open a database', async () => {
    try {
      ...
      // getting first connection
      const sqldb1 = await pool.get(100);
      failTest('got invalid connection');
    } catch (err) {}
  });
  ...
  it('expect getting connection after pool has closed to fail', async () => {
    ...
    try {
      const sqldb1 = await pool.get(100);
      failTest('should not get a connection');
    } catch (err) {}
  });
  ...
  it('expect opening pool multiple times (using different files) to fail', async () => {
    const pool = new SqlConnectionPool();
    try {
      await Promise.all([pool.open('test1.db', SQL_OPEN_DEFAULT, 3), pool.open('test2.db', SQL_OPEN_DEFAULT, 3), pool.open('test3.db', SQL_OPEN_DEFAULT, 3)]);
    } catch (err) {
      failTest(err);
    }
  ...
  it('getting connections from pool having max-limit should succeed', async () => {
    ...
    try {
      ...
      failTest('getting second connection from pool having max==2 should have thrown');
    } catch (err) {}
