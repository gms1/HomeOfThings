...
export class Table {
  ...
  public getCreateIndexStatement(idxName: string, unique?: boolean): string {
    ...
    return (
      'CREATE ' + (unique ? 'UNIQUE ' : ' ') + `INDEX IF NOT EXISTS ${quoteIdentifier(idxName)} ON ${quoteAndUnqualifyIdentifier(this.name)} ` + `(` + idxCols.join(', ') + ')'
    );
  }
  ...
  public createCreateTableStatement(force?: boolean, addFields?: Field[]): string {
    ...
    this.mapNameToFKDef.forEach((fk, fkName) => {
      ...
      stmt += fk.fields.map((field) => quoteSimpleIdentifier(field.foreignColumnName)).join(', ') + ') ON DELETE CASCADE'; // TODO: hard-coded 'ON DELETE CASCADE'
      stmt += '\n';
    });
    stmt += '\n)';
