...
import { FieldOpts, TableOpts } from './decorators';
...
export class MetaModel {
  ...
  setPropertyForeignKey(key: KeyType, constraintName: string, foreignTableName: string, foreignTableField: string): void {
    ...
    propertyFkOpts.set(constraintName, {
      constraintName,
      foreignTableName,
      foreignTableField,
    });
  ...
  init(tableOpts: TableOpts): void {
    ...
    try {
      ...
      throw new Error(`meta model '${this.name}': failed to add table: ${(err as Error).message}`, { cause: err });
    }
        ...
        allPropFkOpts.forEach((propFkOpts, constraintName) => {
          ...
          if (!fkDef) {
            ...
            if (propFkOpts.foreignTableName !== fkDef.foreignTableName) {
              throw new Error(
                `property '${this.name}.${prop.key.toString()}': conflicting foreign table setting: new: '${propFkOpts.foreignTableName}', old '${fkDef.foreignTableName}'`,
              );
            }
          }
          fkDef.fields.push({
